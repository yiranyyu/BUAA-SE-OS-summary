...
1. 如何实现寄存器到寄存器的数据复制？
  * 寄存器 → 寄存器是通过 CPU 的硬件完成
  * 如何实现寄存器到内存的数据复制？
  * 寄存器 → 内存则需要通过分段或者分段和分页机制将逻辑地址转变为物理地址送入地址总线同时发送*写*命令到控制总线，然后将内容通过数据总线写入内存。
2. 如何实现内存到寄存器的数据复制？
...
`call X` 等价于 `push CS; push IP; jmp X`

### 实例理解

#### 键盘驱动下函数交替运行

​		假设内存加载了一个有三个函数的程序，分别是函数A、B，还有一个是键盘中断函数C，键盘按下A时，运行函数A，键盘按下B时，运行函数B。

##### 实现

可以通过将 `A`, `B` 的运行状态备份在内存中并在需要时加载到寄存器中实现

##### 相关问题

1. 三个函数可以共用一个栈吗？
   * 不可以
   * 因为在进行子程序调用或者异常调用时需要将返回值存储在栈上，如果共用一个栈就可能导致返回地址错误的问题
2. 需要多少个栈，如何使用？
   * 至少需要两个栈
   * 在该给定条件下，可以每个函数用一个栈，不过在进行一场调用时需要将现场保存至调用方栈，同时将异常码和返回地址保存到 `C` 的栈中
...
* 首先调用方将需要传递的参数压栈，并压栈需要保存的寄存器
* 通过 `call` 指令压栈返回地址并跳转到目的地址
* 被调用方通过访问栈内容获得参数
* 被调方压栈需要保存的寄存器，比如 `bp` 
...
​		在系统启动时有对 IDT 的初始化，那个地址指向的就是系统调用的总入口函数。这也是系统调用的实现原理。
...
2. CPU 如何从内存中取到指令
  * CPU通过 `CS:IP`得到下一条指令的逻辑地址，将逻辑地址通过分段机制转换为线性地址。如果开启了分页功能的话，则还需要将线性地址通过分页机制映射到物理地址上。从而能够得到对应指令所在的物理内存位置，然后通过总线将指令读取到 CPU 中。
